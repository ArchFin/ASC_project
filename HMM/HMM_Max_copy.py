...
# Post-processing for transition labelling.
min_state_length = 10
...
for i in range(1, len(avg_state_seq)):
    if avg_state_seq[i] != avg_state_seq[i - 1] and (i % session_length) != 1:
        ...
        while start_idx - dynamic_buffer_size_start > 0 and np.abs(avg_fs[start_idx - dynamic_buffer_size_start, new_state] - avg_fs[start_idx - dynamic_buffer_size_start + 1, new_state]) > threshold / 2:
