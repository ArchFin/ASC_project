...
class KMeansVectorClustering:
    ...
    def perform_clustering(self):
        ...
        for _ in range(1000):
            kmeans = KMeans(3)
            kmeans.fit(self.differences_array.iloc[:, -14:])
    ...
    def plot_results(self):
        self.differences_array[["principal componant 1", "principal componant 2"]] = self.differences_array.iloc[:, -14:].dot(self.principal_components)
        plt.scatter(self.differences_array["principal componant 1"], self.differences_array["principal componant 2"], color=self.point_colours, s=0.5)
        ...
        plt.tight_layout()
        plt.savefig(self.savelocation_TET + f'K_means_vector_scatter_plot')
        plt.show()
        ...
        plt.xlabel("principal component 1 (bored*effort)")
        plt.ylabel("principal component 2 (calm)")
        plt.legend(['Cluster {}'.format(i+1) for i in range(cluster_centres_prin.shape[1])])
        plt.title("Cluster centres for K-Means Vector")
        plt.xlim(-1.2, 1.2)
        plt.ylim(-1.2, 1.2)
        plt.tight_layout()
        plt.savefig(self.savelocation_TET + f'cluster_centres_for_kmeans_vectors')
        plt.show()
    ...
    def plot_cluster_centroids(self):
        for i in range(self.cluster_centres_fin.shape[0]):
            ...
            plt.tight_layout()
            plt.savefig(self.savelocation_TET + f'Vector_Cluster_Centroids_{i}')
            plt.show()
    ...
    def stable_cluster_analysis(self):
        ...
        for i in range(cluster_centres_fin_stable.shape[0]):
            ...
            plt.tight_layout()
            plt.savefig(self.savelocation_TET + f'Cluster_Centroids_for_stable_cluster_{i}')
            plt.show()
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(self.savelocation_TET + f'stable_cluster_centroids')
        plt.show()

    def plot_cluster_counts(self):
        self.differences_array['clust_name'] = self.differences_array['clust_name'].astype(str)
        unique, counts = np.unique(self.differences_array['clust_name'], return_counts=True)
        ...
        plt.tight_layout()
        plt.savefig(self.savelocation_TET + f'Cluster_counts')
        plt.show()
...
class KMeansVectorVisualizer:
    ...
    def preprocess_data(self):
        self.differences_array[["principal componant 1 non-diff", "principal componant 2 non-diff"]] = self.differences_array[self.feelings].dot(self.principal_components)
...
class JumpAnalysis:
    ...
    def determine_no_jumps_stability(self):
        ...
        for j in range(1, 30):
            ...
            differences_array_MI['number'] = range(differences_array.shape[0])
        ...
        plt.savefig(self.savelocation_TET + f'stable_cluster_dominance')
        plt.show()
    ...
    def determine_no_jumps_consistency(self):
        ...
        plt.savefig(self.savelocation_TET + f'Cluster_Vectoral_consistency')
        plt.show()
